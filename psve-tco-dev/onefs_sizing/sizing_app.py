...
from tcocal.cal_config import CalConfig
from sizing_lib.aws.page_sizer_connector import SizeSolutionFromPage
...
def show_sizing_results(n_clicks, version, region, workload, capacity, drr, read_tput, write_tput):
    if n_clicks > 0:
        ...
        if len(data_list) != 0: 
            ...
            table_data = [
                {'Message': 1, 'Detail': 'No Valid Sizing Result ro this region, please check the performance and capacity requirement.'}
            ]
            data_table = dash_table.DataTable(
                                        id='datatable',
                                        data=table_data,
